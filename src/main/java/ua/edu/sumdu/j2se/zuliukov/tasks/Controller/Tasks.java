...
public class Tasks {
    public static Iterable<Task> incoming(Iterable<Task> tasks, LocalDateTime start, LocalDateTime end) throws InstantiationException, IllegalAccessException {
        ...
        if (taskIterable instanceof AbstractTaskList) {
            StreamSupport.stream(tasks.spliterator(), false).filter(task -> (task.nextTimeAfter(start) != null && task.nextTimeAfter(start).compareTo(end) <= 0)).forEach(((AbstractTaskList) taskIterable)::add);
        } else {
            StreamSupport.stream(tasks.spliterator(), false).filter(task -> (task.nextTimeAfter(start) != null && task.nextTimeAfter(start).compareTo(end) <= 0)).forEach(((Collection<Task>) taskIterable)::add);
        }
        return taskIterable;
    }
